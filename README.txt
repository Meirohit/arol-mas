...
An agentic system that:
  ...
  - uses an LLM-driven "report agent" (WP3, via the Claude API) that
    ...
    explainable report
  - exposes everything through a CLI (WP4), plus an optional web UI
    (FastAPI backend + React frontend) on top of the same code
...
Then open the Vite dev server URL (default http://localhost:5173). In
dev mode, vite.config.js proxies /api/* and /report-files/* straight
to the FastAPI backend on port 8000, so no CORS setup is needed and
Markdown reports with embedded plot images render correctly.

For a production build: "npm run build" in webapp/ produces
webapp/dist/, which can be served by any static file server; point
its VITE_API_BASE at the deployed backend URL first.
...
-----------------------
  - analytics.torque_expected_range_nm in config.yaml is still a
    placeholder - tune it against real successful-closure torque
    values once you have inspected more of the real dataset.
  - Closure timestamps are approximated as the polling timestamp at
    which the Count increment was observed, not interpolated.
  - Correlation between heads is computed by aligning the i-th
    closure of each head, not by matching timestamps.
  - The closure-events table is not itself dtype-optimized, so a full
    quarter of real data can still reach tens of millions of rows in
    memory during ad-hoc analysis outside the CLI/agent tools.
See docs/architecture.md section 7 for the full, current list.


11. FURTHER DOCUMENTATION
---------------------------
This file covers installation, configuration, and how to run the
program. For everything else:

  docs/architecture.md    System architecture, the full data schema
                          (raw input and internal "events" table),
                          every analytics method, the agent's tool-use
                          decision flow, and an experimental
                          evaluation of the ingestion/analytics layer.

  query_guide.md          Every category of question the agent can
                          answer, worked example commands, and a
                          traceability table mapping each of AROL's
                          own example queries to the tool(s) that
                          answer it.