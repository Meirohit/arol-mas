...
export default function App() {
  ...
  // Date scope, mirroring the CLI's --date (single day) vs
  // --start-date/--end-date (range, can span multiple pool folders) -
  // see cli/main.py::_resolve_date_option. Exactly one mode is active at
  // a time; picking a date in one mode clears the other so a stale value
  // in the hidden field can't silently leak into the request.
  const [dateMode, setDateMode] = useState("pool"); // "pool" | "date" | "range"
  const [singleDate, setSingleDate] = useState("");
  const [startDate, setStartDate] = useState("");
  const [endDate, setEndDate] = useState("");

  // Resolves the current UI state into the { startDate, endDate } shape
  // api.js expects, or undefined when scoped to a plain pool (no date
  // filter at all) - matches _resolve_date_option's semantics without
  // duplicating its validation (the backend still re-validates: this is
  // just what gets sent).
  function activeDateRange() {
    if (dateMode === "date" && singleDate) return { startDate: singleDate, endDate: singleDate };
    if (dateMode === "range" && (startDate || endDate)) return { startDate: startDate || null, endDate: endDate || null };
    return undefined;
  }

  function scopeLabel() {
    if (dateMode === "date" && singleDate) return singleDate;
    if (dateMode === "range" && (startDate || endDate)) return `${startDate || "…"} → ${endDate || "…"}`;
    return pool || "—";
  }
  ...
  async function send() {
    ...
    setBusy(true);
    try {
      const report = await api.ask(query, pool, activeDateRange());
  ...
  async function runPresetDirect(kind) {
    ...
    setBusy(true);
    try {
      const report = await api.runReport(kind, pool, activeDateRange());
      setMessages((m) => [...m.slice(0, -1), { role: "report", report }]);
    } catch (e) {
      setMessages((m) => [...m.slice(0, -1), { role: "error", text: e.message }]);
    } finally {
    ...
    <div className="app">
      <aside className="sidebar">
        ...
        <div className="panel-body">
          <div>
            ...
            <select value={pool} onChange={(e) => setPool(e.target.value)} disabled={dateMode !== "pool"}>
              {pools.map((p) => (
          ...
          <div>
            ...
            <div className="date-mode-row">
              <label className="radio-pill">
                <input type="radio" name="date-mode" checked={dateMode === "pool"}
                  onChange={() => setDateMode("pool")} />
                Whole pool
              </label>
              <label className="radio-pill">
                <input type="radio" name="date-mode" checked={dateMode === "date"}
                  onChange={() => setDateMode("date")} />
                Single day
              </label>
              <label className="radio-pill">
                <input type="radio" name="date-mode" checked={dateMode === "range"}
            ...
            {dateMode === "range" && (
              <p className="hint hint-tight">
                A range can span multiple pool folders (e.g. late Feb into early
                March) - leave either end blank to leave that side open.
              </p>
            )}
          </div>
      ...
      <main className="chat">
        <div className="statusbar">
          ...
          <span>{dateMode === "pool" ? "POOL" : "SCOPE"} <strong>{scopeLabel()}</strong></span>
        </div>
