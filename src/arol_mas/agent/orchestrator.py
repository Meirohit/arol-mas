    ...
    def run(self, user_query: str, dataset_label: str = "") -> ReportResult:
        ...
        for _iteration in range(self.settings.agent.max_tool_iterations):
            ...
            for block in tool_use_blocks:
                ...
                # Capture plot_path here, from the ONE real execution of a
                # plot_* tool, so callers (webapi/server.py) can build the
                # web UI's chart list from this instead of re-running the
                # same plot_* tool a second time just to get another copy
                # of the PNG. Re-running it was strictly worse: a fresh
                # timestamped file (so the "same" chart looked different
                # from the one already embedded inline in the report text)
                # and double the plotting cost on every chart question.
                # Only plot_path (a short string) is captured, not the
                # full tool result - other tools' results can contain
                # numpy/pandas scalar types that json.dumps(default=str)
                # tolerates here but a plain FastAPI response wouldn't.
                plot_path = result.get("plot_path") if isinstance(result, dict) else None
                tool_calls_log.append({"tool": block.name, "input": block.input, "plot_path": plot_path})
                tool_results.append({
                    "type": "tool_result",
