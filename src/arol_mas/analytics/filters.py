...
def resolve_end_of_range(end_date, end_ts: pd.Timestamp) -> pd.Timestamp:
    """
    Single source of truth for "was `end_date` a bare date (extend to
    end-of-day) or an explicit timestamp (treat as an exact instant)?" -
    shared by this module and agent/tools.py::_scope_events so the two
    code paths can't silently disagree on the same edge case again.

    Decided by the STRING LENGTH of the original input ("2026-03-05" is
    10 chars, a bare date; "2026-03-05T00:00:00Z" is 20 chars, an
    explicit timestamp) - never by inspecting the parsed time-of-day.
    Inspecting the parsed time only tells you the clock reads midnight;
    it can't tell an *implicit* midnight (from a bare date, meant to mean
    "the whole day") from an *explicit* one (the caller asked for that
    exact instant, and stretching it to 23:59:59 would silently return
    24h more data than requested).
    """
