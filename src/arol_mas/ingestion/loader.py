...
# In-memory cache of already-streamed (events, idle_periods, meta) results,
# keyed on the exact resolved file set (path + mtime + size, so editing/
# replacing/adding a file invalidates its entry automatically without any
# explicit "clear cache" step). Every question asked against the same
# pool/date-range in a running server previously re-read and re-parsed
# every raw CSV/JSON/Parquet file from scratch - ~73s for one month of
# real AROL data even though nothing on disk had changed - because
# load_pool_streaming/load_period_streaming had no caching at all. Since
# _stream_files is the shared core of both, caching it here benefits both
# the CLI and the web server, and correctly shares one cached result
# between a single-pool request and an equivalent date-range request that
# happens to resolve to the exact same files.
#
# Bounded to a handful of entries (LRU-evicted) rather than unbounded,
# since each entry holds a full events/idle_periods DataFrame in memory -
# fine for a few recently-used pools/ranges, not fine to keep every one
...
def _stream_cache_key(files: List[Path]) -> tuple:
    """(path, mtime, size) per file - changes if any file is edited,
    replaced, or if the resolved file set itself differs, so a stale
    cache entry can never be served for changed data."""
    key = []
    for p in files:
        st = p.stat()
...
def _stream_files(files: List[Path], settings: Settings, strict: bool, label: str) -> Tuple[pd.DataFrame, pd.DataFrame, dict]:
    ...
    Results are cached by the resolved file set (see _stream_cache_key) -
    repeating the same pool/date-range request re-parses nothing as long
    as the underlying files are unchanged. strict=True bypasses caching:
    it's used by callers that want validation errors to raise, which is
    rare enough (not the hot path) that it isn't worth the cache-key
    complexity of also tracking `strict` as a cache dimension.
    """
    if not strict:
